...
建议定期清理以保持游戏流畅。`, function(confirmed) {
        ...
        try {
            ...
            showNotification('清理成功', `已清理 ${cleanedCount} 个缓存项，存档内存已优化`);
...
function confirmAutoCleanCacheChange() {
    ...
    // 如果在存档管理窗口中，更新显示
    const valueEl = document.querySelector('#archiveManagementModal .settings-item:nth-child(4) .settings-value');
    if (valueEl) {
    ...
    // 显示通知
    showNotification('设置已保存', `自动清理缓存已设置为：${intervalText}`);
